...
int ZlibStream::init(Type type,
                     ...
                     Strategy strategy) {
    // 压缩等级
    ANCFL_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    // 选择模式
    ANCFL_ASSERT((window_bits >= 8 && window_bits <= 15));
    // 运行过程中的内存限制
    ANCFL_ASSERT((memlevel >= 1 && memlevel <= 9));
...
ancfl::ByteArray::ptr ZlibStream::getByteArray() {
