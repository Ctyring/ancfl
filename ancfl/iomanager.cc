...
IOManager::FdContext::EventContext& IOManager::FdContext::getContext(
    ...
    switch (event) {
        ...
        case IOManager::WRITE:
            return write;
        default:
            ANCFL_ASSERT2(false, "getContext");
    }
...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    ANCFL_ASSERT(!event_ctx.scheduler && !event_ctx.fiber && !event_ctx.cb);
    ...
    if (cb) {
        ...
        ANCFL_ASSERT2(event_ctx.fiber->getState() == Fiber::EXEC,
...
void IOManager::idle() {
    ...
    while (true) {
        ...
        // 统一处理
        if (!cbs.empty()) {
            // ANCFL_LOG_DEBUG(g_logger) << "on timer cbs.size=" <<
            // cbs.size();
